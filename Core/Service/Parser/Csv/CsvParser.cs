...
public sealed class CsvParser : ISaleParser
{
    public Sale ParseToSale(string saleString)
    {
        ...
        try
        {
            ...
            if (!csv.Read())
            {
                throw new SaleParsingException("Failed to parse sale: CSV row is empty", new FormatException());
            }
        ...
        {
            throw new SaleParsingException($"Failed to parse sale: cannot convert value '{typeException.Text}' to the required type", typeException);
        }
        catch (CsvHelper.MissingFieldException ex)
        {
            throw new SaleParsingException("Failed to parse sale: one or more required CSV fields are missing", ex);
        }
        catch (BadDataException ex)
        {
            throw new SaleParsingException("Failed to parse sale: invalid CSV format", ex);
        }
